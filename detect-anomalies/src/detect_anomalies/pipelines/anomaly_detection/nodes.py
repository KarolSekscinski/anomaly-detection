...
def consume_kafka_data(topic_name: str, bootstrap_servers_config: dict) -> pd.DataFrame:
    ...
    if os.getenv("is_docker"):
        bootstrap_servers = f"{bootstrap_servers_config['host']}:2{bootstrap_servers_config['port']}"
    else:
...
def produce_kafka_data(
        topic_name, bootstrap_servers_config: dict,
        anomalies_high_volume: pd.DataFrame,
        anomalies_low_volume: pd.DataFrame,
        anomalies_using_isolation_forest: pd.DataFrame
) -> None:
    """
    ...
    Args:
        ...
        bootstrap_servers_config (dict): A dictionary containing Kafka bootstrap server configuration
            (e.g., {'host': 'kafka-broker', 'port': 29092}).
        anomalies_high_volume (pd.DataFrame): The data to be produced to Kafka.
        anomalies_low_volume (pd.DataFrame): The data to be produced to Kafka.
        anomalies_using_isolation_forest (pd.DataFrame): The data to be produced to Kafka.
    Returns:
        None
    ...
    list_of_dataframes_to_publish = [anomalies_high_volume, anomalies_low_volume, anomalies_using_isolation_forest]
    for data in list_of_dataframes_to_publish:
